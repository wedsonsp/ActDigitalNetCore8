    ...
    namespace Ambev.DeveloperEvaluation.ORM.Repositories
    {
        ...
        public class VendaRepository : IVendaRepository
        {
            ...
            public async Task<Venda> CreateAsync(Venda venda, CancellationToken cancellationToken = default)
            {
                ...
                var itensAgrupados = itensVenda
                    .GroupBy(i => i.IdProduto) // Agrupar por produto
                    .ToList();

                foreach (var grupo in itensAgrupados)
                {
                    var quantidadeTotal = grupo.Sum(i => i.Quantidade);  // Soma todas as quantidades do produto

                    if (quantidadeTotal > 20)
                    {
                        throw new ArgumentException($"Não é permitido vender mais de 20 unidades por total de produtos comprados {grupo.Key}. A quantidade total excede 20.");
                    }
                ...
                venda.ItensVenda = itensAgrupados.Select(g => g.First()).ToList(); // Mantém um único item por produto
